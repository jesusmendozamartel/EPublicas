(function() {
  ...
  callWithJQuery(function($) {
    ...
    Utilities
     */
    var PivotData, addSeparators, aggregatorTemplates, aggregators, dayNamesEn, derivers, getSort, locales, mthNamesEn, naturalSort, numberFormat, pivotTableRenderer, renderers, sortAs, usFmt, usFmtInt,usTasa, usFmtPct, usFmtPctMostrar, zeroPad;
    ...
    usFmtPctMostrar = numberFormat({
      digitsAfterDecimal: 1,
      suffix: "%"
    });
    sin_simbolo = numberFormat({
      digitsAfterDecimal: 1
    });
    ...
    aggregatorTemplates = {
        ...
        return function(arg) {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              value: function() {
                return this.sum <= 0 ? 0.001 :this.sum;
              },
              format: formatter,
              numInputs: attr != null ? 0 : 1
            };
      ...
      porcentaje_mostrar: function(formatter) {
        if (formatter == null) {
          formatter = usFmtPctMostrar;
        }
        return function(arg) {
          var attr;
          attr = arg[0];
      ...
      //porcentaje
      porcentaje: function(wrapped, type, formatter) {
       if (type == null) {
          type = "total";
        }
        if (formatter == null) {
        ...
        return function() {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              format: formatter,
              value: function() {
                //console.log(this.inner.value(), this.selector[1][0],data);
      ...
      porcentaje_acumulado: function(wrapped, type, formatter) {
       if (type == null) {
          type = "total";
        }
        if (formatter == null) {
        ...
        return function() {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              format: formatter,
              value: function() {
                /*if(this.selector[1][0]!=undefined)
                    {
                    if(!window["variable" + this.selector[1][0]])
                        {
                        eval(" var variable" + this.selector[1][0]);
                        window["variable" + this.selector[1][0]] = this.inner.value();
                        }
                    else
                        eval("variable" + this.selector[1][0] + "+="+this.inner.value());
                    console.log("variable" + this.selector[1][0]+'='+window["variable" + this.selector[1][0]]);

                    }
                    resultado=window["variable" + this.selector[1][0]]/ data.getAggregator.apply(data, this.selector).inner.value();
                    if(resultado==3||resultado==2||resultado==1)
                        window["variable" + this.selector[1][0]]=undefined;
    ...
    //   },
      /*"Mapa de Calor por Filas": function(pvtData, opts) {
        return $(pivotTableRenderer(pvtData, opts)).heatmap("rowheatmap");
    },*/
      "Mapa de Calor por Columnas": function(pvtData, opts) {
    ...
    pivotTableRenderer = function(pivotData, opts) {
      ...
      for (j in colAttrs) {
        ...
        if (parseInt(j) === 0) {
          ...
          if(colAttrs=='')
              ...
              th.className = "pvtTotalLabel oculto"; //oculto
      ...
      if (rowAttrs.length !== 0) {
        ...
        if (colAttrs.length === 0) {
          th.className = "pvtTotalLabel";//oculto
      ...
      for (i in rowKeys) {
        ...
        if(colKeys=='')
            ...
            td.className = "pvtTotal rowTotal oculto"; //oculto
      ...
      for (j in colKeys) {
        ...
        //if(val==2)
        //	{
            //td.className = "pvtTotal oculto colTotal";
            //$(".pvtTotalLabel").addClass("oculto");
        //	}
        //else
        //	{
            td.className = "pvtTotal colTotal";
            //$(".pvtTotalLabel").removeClass("oculto");
        //	}
        td.innerHTML = totalAggregator.format(val);
        td.setAttribute("data-value", val);
      ...
      if(colKeys=='')
        ...
        td.className = "pvtGrandTotal oculto";
      td.innerHTML = totalAggregator.format(val);
      td.setAttribute("data-value", val);
      tr.appendChild(td);
      result.appendChild(tr);
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
      ...
      try {
        ...
        uiTable = $("<table>", {
          "class": "pvtUi table table-condensed"
        }).attr("cellpadding", 5);
        rendererControl = $("<td>");
        renderer = $("<select>").addClass('pvtRenderer form-control width-6').appendTo(rendererControl).bind("change", function() {
          return refresh();
        });
        ref1 = opts.renderers;
        for (x in ref1) {
        ...
        aggregator = $("<select>").addClass('pvtAggregator form-control width-6').bind("change", function() {
            //sumatoria();
          return refresh();
        });
        ...
        sumatoria = (function(_this) {
          return function() {
            if($('.pvtAggregator').val()=="Porcentaje Acumulado")
            {
                $("#pvtTable  td.pvtTotal.colTotal").each(function (index)
                {
                    cantidad=$(this).text();
                    cantidad=cantidad.replace(/\s/gi,'');
                    window["t" + index]=parseFloat(cantidad);
                });
                $("#pvtTable  tr").each(function (index)
                {
                    $(this).children("td.pvtVal").each(function (index2)
                    {
                    cantidad=$(this).text();
                    cantidad=cantidad.replace(/\s/gi,'');
                    if(window["v" + index2])
                        eval("v"+index2+"+="+parseFloat(cantidad));
                    else
                        window["v" + index2]=parseFloat(cantidad);
                    $(this).html('hola');
                    console.log($(this).text()+' - '+window["v" + index2]+' - '+window["t" + index2]);
                    });
                });
            }
          };
        ...
        refresh = (function(_this) {
          return function() {
            ...
            //$("#output").html("<div align='center'><img src='"+BASE_URL +"assets/img/loading.gif' id='ajax-loader'  /><br/><span style='font-size:10px; font-family:arial;'>Cargando</span></div>");
