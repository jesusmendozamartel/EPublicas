...
<div align="center">
  ...
  <br />
  <strong><font face="Arial" size='3pt' color='#000000'>Información Financiera a nivel de CIIU Rev.4 </font></strong>- En Nuevos Soles</div>
<form action="InfFinancieraciiu.asp" name="fmrEF" method="post" target=_self>
  ...
  <div name="formulario" id="formulario1">
	...
	<select name="cboGrupo" id="cboGrupo" class="combo2" >
	  <option value="" selected><%Response.Write "[Seleccione]"%></option>
		<%  
		SQL = "SELECT DISTINCT GRUPO FROM Sunatpro_InfFinancieraCIIU WHERE GRUPO <> 'G5' ORDER BY 1"
			Set rs = Server.CreateObject("ADODB.Recordset")
			rs.Open SQL , con
			If Not rs.BOF Then rs.MoveFirst
			Do While Not rs.EOF			
	  ...
	  <option value="<%=Trim(rs("GRUPO"))%>"<%If Trim(CStr(strUbicaGrupo)) = Trim(CStr(rs("GRUPO"))) Then%>selected<% End If%>><%=Trim(rs("GRUPO"))%> </option>
		...
		&nbsp;&nbsp;
		<%
			strOnchange2="cargaVariable(1,'IF');"
			strOnchange3="Excel(1,'IF');"
		%>
		...
		<button onClick="Refresh(2)" style=" border:none; height:21px; width:21Px;font-weight:bold;font-size:8pt;background-color:#ffffff;color:#123456;">
